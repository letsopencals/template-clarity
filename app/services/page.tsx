...
function ServiceCard({ product, index }: { product: ProductListItemResponse; index: number }) {
	...
	// The bookable unit is always a variant; product-level fields are display-only fallbacks.
		...
		>
			...
			<div className="p-7 lg:p-9">
				<div className="flex items-start justify-between gap-6">
					<div>
						<h2 className="heading-display text-3xl text-[var(--color-ink)] md:text-4xl">
							{product.title}
						</h2>
						{product.description && (
					...
					<div className="mt-7 space-y-2">
						{variants.map((variant) => (
							<Link
								...
								className="group flex items-center justify-between rounded-xl border border-[var(--color-line)] bg-[var(--color-bg)]/40 p-4 transition-all hover:border-[var(--color-primary)]/40"
							>
								<div>
									<p className="text-sm font-medium text-[var(--color-ink)]">{variant.variantTitle}</p>
								...
								<div className="flex items-center gap-4">
									...
									<svg
										className="h-4 w-4 text-[var(--color-ink-dim)] transition-transform group-hover:translate-x-1 group-hover:text-[var(--color-primary)]"
										fill="none"
										viewBox="0 0 24 24"
										stroke="currentColor"
										strokeWidth={2}
					...
					<>
						<div className="mt-7 flex flex-wrap items-center gap-6 text-sm">
							...
							<div className="flex items-center gap-2 text-[var(--color-ink-muted)]">
								...
								<span className="font-semibold text-[var(--color-primary)]">{formatPrice(primary?.price ?? product.price, currency)}</span>
							</div>
						...
						<Link
							...
							className="group mt-7 inline-flex items-center gap-3 rounded-full bg-[var(--color-primary)] px-7 py-3.5 text-[0.72rem] font-semibold uppercase tracking-[0.22em] text-[var(--color-bg)] transition-all hover:bg-[var(--color-primary-bright)]"
						>
							Book Now
							<svg
				...
				<button
					...
					className={`rounded-full border px-5 py-2.5 text-[0.7rem] font-semibold uppercase tracking-[0.18em] transition-all ${
						selectedLocationId === null
							...
							: 'border-[var(--color-line-strong)] text-[var(--color-ink)] hover:border-[var(--color-ink)]/40'
					}`}
				>
					All Locations
				</button>
		...
		<>
			...
			<section id="services-hero" className="bg-[var(--color-bg)] pt-32 pb-16 lg:pt-40 lg:pb-20">
				<div className="mx-auto max-w-[1400px] px-6 lg:px-10">
					...
					{/* Prefer browsing by department; this page lists every service in one place. */}
					<motion.h1
						initial={{ opacity: 0, y: 30 }}
						animate={{ opacity: 1, y: 0 }}
					...
					>
						Our
						<br />
						<span className="heading-display-italic text-[var(--color-primary)]">services</span>
					</motion.h1>
					<motion.p
						initial={{ opacity: 0, y: 20 }}
						animate={{ opacity: 1, y: 0 }}
					...
					>
						Every service across our nine departments, in one place. Prefer to browse by
						specialty? Visit <Link href="/departments" className="text-[var(--color-primary)] underline underline-offset-4">departments</Link>.
					</motion.p>
			...
			<section id="services-list" className="bg-[var(--color-bg)] pb-20 lg:pb-32">
				<div className="mx-auto max-w-[1400px] px-6 lg:px-10">
					...
					{!loading && !error && products.length === 0 && (
						<div className="py-20 text-center">
							<p className="text-[var(--color-ink-muted)]">No services available at the moment.</p>
						</div>
					)}
			...
			{/* Bottom CTA */}
			<section id="services-cta" className="bg-[var(--color-primary)] py-20">
				<div className="mx-auto max-w-[1400px] px-6 text-center lg:px-10">
					...
					<p className="mx-auto mt-4 max-w-md text-base text-[var(--color-bg)]/75">
						Get in touch — our team will point you to the right department and clinician.
					</p>
					<div className="mt-8">
						<Link
							href="/contact"
							className="inline-flex items-center gap-3 rounded-full bg-[var(--color-bg)] px-10 py-4 text-[0.72rem] font-semibold uppercase tracking-[0.22em] text-[var(--color-ink)] transition-all hover:bg-[var(--color-bg-deep)]"
						>
							Contact Us
						</Link>
					</div>
