			...
			<div className="mx-auto max-w-[1400px] px-6 lg:px-10">
				<div className="grid items-center gap-16 lg:grid-cols-2">
					...
					>
						...
						<div className="absolute -bottom-4 left-0 h-1 w-24 bg-[var(--color-primary)]" />
					</motion.div>
					...
					<div>
						...
						<motion.h2
							...
							className="heading-display mt-4 text-4xl text-[var(--color-ink)] md:text-5xl"
						>
							Built around
							<br />
							<span className="heading-display-italic text-[var(--color-primary)]">the patient.</span>
						</motion.h2>
						<motion.div
							initial={{ opacity: 0, y: 20 }}
							animate={isInView ? { opacity: 1, y: 0 } : {}}
		...
		<section ref={ref} className="noise-overlay relative bg-[var(--color-bg-deep)] py-section-sm lg:py-section">
			<div className="relative z-10 mx-auto max-w-[1400px] px-6 lg:px-10">
				...
				<motion.h2
					...
					className="heading-display mt-4 text-5xl text-[var(--color-ink)] md:text-6xl"
				>
					Our values
				</motion.h2>
				...
				<div className="mt-16 grid gap-px overflow-hidden rounded-2xl bg-[var(--color-line)] md:grid-cols-2">
					{siteConfig.values.map((value, i) => (
						...
						>
							<span className="font-display text-sm text-[var(--color-primary)]">{value.number}</span>
							<h3 className="heading-display mt-4 text-2xl text-[var(--color-ink)]">{value.title}</h3>
		...
		<section ref={ref} id="team" className="bg-[var(--color-bg)] py-section-sm lg:py-section">
			<div className="mx-auto max-w-[1400px] px-6 lg:px-10">
				...
				>
					Meet the
					<br />
					<span className="heading-display-italic text-[var(--color-primary)]">clinicians</span>
				</motion.h2>
				...
				<div className="mt-16 grid gap-8 md:grid-cols-2 lg:grid-cols-4">
					{siteConfig.team.map((member, i) => (
						...
						>
							...
							<h3 className="mt-6 font-display text-xl font-semibold text-[var(--color-ink)]">
								{member.name}
							</h3>
		...
		<>
			...
			<section className="relative bg-[var(--color-bg)] pt-32 pb-20 lg:pt-40 lg:pb-28">
				<div className="mx-auto max-w-[1400px] px-6 lg:px-10">
					...
					>
						...
						<span className="heading-display-italic text-[var(--color-primary)]">{about.heroHeadingAccent}</span>
					</motion.h1>
					<motion.p
						initial={{ opacity: 0, y: 20 }}
						animate={{ opacity: 1, y: 0 }}
			...
			<section className="bg-[var(--color-bg)] py-20 lg:py-28">
				<div className="mx-auto max-w-[1400px] px-6 text-center lg:px-10">
					<h2 className="heading-display text-4xl text-[var(--color-ink)] md:text-5xl">
						{about.bottomCta}
					</h2>
					<p className="mx-auto mt-4 max-w-md text-base text-[var(--color-ink-muted)]">
						{about.bottomCtaBody}
					</p>
					<div className="mt-8">
						<Link
							href="/services"
							className="inline-flex items-center gap-3 rounded-full bg-[var(--color-primary)] px-10 py-4 text-[0.72rem] font-semibold uppercase tracking-[0.22em] text-[var(--color-bg)] transition-all hover:bg-[var(--color-primary-bright)]"
						>
							Book Now
						</Link>
					</div>
